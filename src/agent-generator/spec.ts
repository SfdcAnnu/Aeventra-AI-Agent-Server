/**
 * The generator's knowledge of "what Archon can build with" â€” a deliberate,
 ...
 * nodes/*.ts). Keeping three hand-synced copies is a real cost â€” see the
 * plan doc for why this isn't unified in this pass. If you add/change a
 * node type anywhere else, update this file too.
 *
 * Only node subtypes that are PROVEN, live-wired executors are listed â€”
...
export const NODE_SPEC: NodeSpecEntry[] = [
  ...
  },
  {
    ...
    when: 'Use for any step that needs judgment, scoring, summarization, or deciding which tool to call â€” not for simple deterministic CRUD (use action nodes for that).',
    ports: ['out'],
    fields: [
      ...
      { key: 'instruction', type: 'textarea', description: 'Plain-English instruction. This is the most important field â€” be specific about what to look up, what to decide, and what action to take or recommend. Reference {!record.Field} for trigger data.', example: 'Look up the {!record.Company} and {!record.Industry}. Score 0-100 how likely this lead is to convert. Give one short sentence of reasoning.' },
  ...
  { type: 'ai', subType: 'gpt4', label: 'GPT-4 (AI step)', when: 'Same as claude â€” use only if the user specifically asked for OpenAI/GPT.', ports: ['out'],
    fields: [
  ...
  { type: 'ai', subType: 'gemini', label: 'Gemini (AI step)', when: 'Same as claude â€” use only if the user specifically asked for Gemini/Google AI.', ports: ['out'],
    fields: [
  ...
  },
  {
    ...
    fields: [
      ...
      { key: 'template', type: 'text', description: 'The value â€” plain text mixed with {!variable} references.', example: 'Lead {!record.Name} scored {!ai.score}' },
    ],
  },
  {
    type: 'logic', subType: 'wait', label: 'Wait',
    when: 'Pause before continuing. Durable â€” survives a server restart for waits longer than 60 seconds.',
    ports: ['out'],
    fields: [
  ...
  },
  {
    ...
    fields: [
      { key: 'approverField', type: 'text', description: 'A field on the TRIGGER object that resolves to a User Id â€” a direct field (OwnerId) or a relationship path (Owner.ManagerId).', example: 'OwnerId' },
  ...
  },
  {
    ...
    when: 'Repeat a set of steps once per item in a list (e.g. a list of records from a query_records node). The "each" port is the loop body (runs once per item); the "done" port runs once after all iterations finish. IMPORTANT: never put a wait, approval, or another loop node inside the each-port body â€” not supported and will fail validation.',
    ports: ['each', 'done'],
    fields: [
      { key: 'collectionVar', type: 'text', description: 'A {!variable} that resolves to a list â€” typically {!action.records} right after a query_records node (the alias is the upstream node\'s TYPE, "action", not its subtype).', example: '{!action.records}' },
  ...
  { type: 'action', subType: 'update_record', label: 'Update Record', when: 'Deterministically update fields on a record â€” the trigger record by default.', ports: ['out'],
  ...
  { type: 'action', subType: 'query_records', label: 'Query Records', when: 'Run a SOQL query and get back a list â€” the natural upstream of a Loop node.', ports: ['out'],
  ...
  { type: 'action', subType: 'create_task', label: 'Create Task', when: 'Shortcut for creating a Salesforce Task â€” simpler than create_record for this specific, very common case.', ports: ['out'],
    ...
    fields: [ { key: 'message', type: 'text', description: 'Message text, can use {!variable}.' } ] },
  {
    ...
    when: 'THE way to take action through a connected external provider (email, Slack, storage, or even a Salesforce MCP tool) deterministically â€” no AI judgment, just call one specific tool with specific parameters. This replaces per-provider node types; do not invent an "outlook" or "slack" node type, always use call_tool with the right provider.',
    ports: ['out'],
    fields: [
      { key: 'provider', type: 'text', description: 'A provider key â€” see the CONNECTED PROVIDERS list below for what this org has. Use the closest match even if not yet connected (e.g. "gmail" for email) â€” connecting it becomes a checklist item.' },
      { key: 'connectorId', type: 'text', description: 'Leave empty string â€” the user binds this after connecting the provider.' },
      ...
      { key: 'toolName', type: 'text', description: 'The specific tool name. For gmail: sendEmail, listEmails, readEmail, searchEmails, replyEmail, createDraft. For slack-like channel providers: postMessage. Best-guess a sensible name if unsure â€” the user can correct it via the tool picker, which shows the real live list.' },
      { key: 'paramValues', type: 'json-text', description: 'JSON object mapping the tool\'s real parameter names to values (which can contain {!variable} tokens). For gmail sendEmail: {"to":"[\\"{!record.Email}\\"]","subject":"...","body":"..."} â€” note "to" is a JSON array, so its value must itself be a JSON-array STRING.' },
    ],
  },
  ...
  { type: 'end', subType: 'end', label: 'End', when: 'Optional terminal marker â€” most agents do not need one; only add it if the requirement explicitly wants execution logged.', ports: [],
    fields: [ { key: 'logExecution', type: 'toggle', description: 'true to log this run to Salesforce.' } ] },
];

/**
 * Chat-mode's vocabulary â€” a live, real-time conversational agent (Slack/
...
export const CHAT_NODE_SPEC: NodeSpecEntry[] = [
  {
    ...
    when: 'Every chat agent needs EXACTLY ONE of these, as the first node (index 0) â€” the agent the user actually talks to. Its own subagent/tool/catalog children (if any) connect FROM this node on its "tool" port.',
    ports: ['tool'],
    fields: [
      { key: 'model', type: 'picklist(claude-opus-4-7,claude-sonnet-4-6,claude-haiku-4-5)', description: 'Default to claude-sonnet-4-6 unless the task clearly needs top reasoning quality (opus) or is trivial/high-volume (haiku).' },
      { key: 'systemPrompt', type: 'textarea', description: 'The agent\'s full persona and instructions â€” tone, what it can/should do, any hard rules (e.g. never quote a price without approval). This is the most important field.' },
      { key: 'useKnowledgeBase', type: 'toggle', description: 'true if this agent should ground answers in the agent-level Knowledge Base.' },
    ],
  },
  { type: 'ai', subType: 'gpt4', label: 'GPT-4 (top-level agent)', when: 'Same as claude â€” use only if the user specifically asked for OpenAI/GPT.', ports: ['tool'],
    fields: [
      { key: 'model', type: 'picklist(gpt-4o,gpt-4o-mini,gpt-4-turbo,gpt-4.1,gpt-4.1-mini)', description: 'Default gpt-4o.' },
      { key: 'systemPrompt', type: 'textarea', description: 'Same as claude.systemPrompt.' },
      { key: 'useKnowledgeBase', type: 'toggle', description: 'Same as claude.useKnowledgeBase.' },
    ] },
  { type: 'ai', subType: 'gemini', label: 'Gemini (top-level agent)', when: 'Same as claude â€” use only if the user specifically asked for Gemini/Google AI.', ports: ['tool'],
    fields: [
      { key: 'model', type: 'picklist(gemini-2.5-pro,gemini-2.5-flash,gemini-2.0-flash,gemini-2.0-flash-lite)', description: 'Default gemini-2.5-flash.' },
      { key: 'systemPrompt', type: 'textarea', description: 'Same as claude.systemPrompt.' },
      { key: 'useKnowledgeBase', type: 'toggle', description: 'Same as claude.useKnowledgeBase.' },
    ] },
  {
    ...
    when: 'A Level-2 specialist the top-level agent can HAND OFF the conversation to for one specific domain (e.g. "Billing", "Escalation to Human", "Returns") â€” its own independent system prompt and model, not just an instruction tweak. Only add these when the requirement clearly describes distinct conversation modes/topics; a simple single-purpose agent needs none. Connects FROM the top-level ai node\'s "tool" port. A subagent may have its OWN attached tool nodes (also via its "tool" port) but cannot hand off to a further subagent â€” one level of handoff only.',
    ports: ['tool'],
    fields: [
      { key: 'model', type: 'text', description: 'Leave "" to inherit the top-level agent\'s provider/model â€” the common case. Only set claude/gpt4/gemini explicitly if this specialist genuinely needs a different one.' },
      { key: 'routingDescription', type: 'text', description: 'Shown to the TOP-LEVEL model as this subagent\'s "tool" description â€” this is what actually drives when it gets picked, so be specific about what belongs here vs. other subagents.', example: 'Handles billing questions, invoice lookups, and payment disputes.' },
  ...
  },
  {
    ...
    when: 'ONE specific, named, callable action the agent (or a subagent) can invoke mid-conversation â€” e.g. "look up an order", "create a support ticket", "escalate to a human". Connects FROM the ai or subagent node it belongs to, on that node\'s "tool" port. Different from a catalog node (a whole browsable toolset) â€” use tool nodes for a small number of specific, named actions the requirement calls out explicitly.',
    ports: [],
    fields: [
      { key: 'description', type: 'text', description: 'Shown to the model â€” what this tool does and when to call it.' },
      ...
      { key: 'toolName', type: 'text', description: 'The specific tool name, best-guessed from the CONNECTED PROVIDERS list â€” the user corrects it via the tool picker if needed.' },
      { key: 'connectorId', type: 'text', description: 'Leave empty string â€” the user binds this after connecting the provider.' },
  ...
  { type: 'catalog', subType: 'channel_tools', label: 'Channel Tools (catalog)', when: 'Attach to the ai or subagent node it belongs to so it can post to chat channels itself mid-conversation.', ports: [],
    fields: [ { key: 'description', type: 'text', description: 'Shown to the AI.' }, { key: 'connectorId', type: 'text', description: 'Leave empty string.' }, { key: 'allowedTools', type: 'string[]', description: 'Subset of: list_channels, list_users, read_channel_history, post_message, update_message, add_reaction, upload_file.' } ] },
  // Guardrail/automation spec entries removed at the user's request
  // (2026-09-03) — the copilot no longer authors these node types.
];
...
function renderNodeBlock(spec: NodeSpecEntry[]): string {
  return spec.map((n) => {
    const fields = n.fields.length
      ...
      : '    (no config fields)';
    return `- type="${n.type}" subType="${n.subType}" ("${n.label}") â€” ports: [${n.ports.join(', ') || 'none, terminal'}]\n  When to use: ${n.when}\n  Config fields:\n${fields}`;
  }).join('\n\n');
}

function renderProviderBlock(providerStatus: Array<{ key: string; connected: boolean }>): string {
  return providerStatus.length
    ? providerStatus.map((p) => `- ${p.key}${p.connected ? ' (connected)' : ' (not connected yet â€” using it is fine, it becomes a checklist item)'}`).join('\n')
    : '(no providers configured in this org yet â€” any provider you reference becomes a checklist item)';
}
 ...
 *  graphs (see CHAT_NODE_SPEC's doc comment) â€” sharing one prompt across both
...
export function buildSystemPrompt(
  ...
  if (mode === 'chat') {
    return `You are Archon's agent-generation assistant. You turn a plain-English requirement into a complete, valid Archon CHAT agent graph â€” a live conversational agent (e.g. a WhatsApp/Slack/web chat assistant), NOT a record-triggered automation.
...
- CRITICAL: every connection whose target is a "subagent" or "tool" node MUST use fromPort="tool" exactly â€” this is not stylistic, it is the literal port name server/src/chat/subagent-router.ts matches on. Any other fromPort value makes that subagent/tool invisible and uncallable at runtime even though it renders as connected. Catalog node connections should also use fromPort="tool" for consistency, though it is not strictly required for them.
- Only add subagent nodes when the requirement clearly describes genuinely distinct conversation domains/specialists. Most requirements need just the one top-level ai node plus its directly-attached tool/catalog nodes â€” do not invent subagents to seem thorough.
- Be PROACTIVE: if the requirement clearly implies structure it didn't spell out (e.g. "answer questions from our FAQ" implies useKnowledgeBase=true; "look up order status" implies a tool node against the best-matching connected provider), include it â€” but set a one-sentence "rationale" on that specific node explaining why you added it. Do not invent unrelated features.
- Never fabricate a provider that isn't Salesforce/email/storage/channel in nature â€” if the requirement needs something Archon genuinely can't do, omit it and explain the gap in the setup checklist instead of inventing a fake node type.
- SECURITY: every ai/subagent systemPrompt must scope the agent to THIS conversation's own customer/record â€” reads AND writes. The agent must refuse to list/enumerate/export other customers' or org-wide records ("all opportunities", "every account", etc.), however the request is phrased. Write this boundary into the prompt explicitly (a canonical DATA BOUNDARY block is also appended automatically as a safety net).

Decide whether to call ask_clarifying_questions or create_agent. Only ask questions when truly blocked â€” prefer a reasonable default plus a checklist note over asking.`;
  }
...
- {!record.FieldName} â€” a field on the record that triggered the run (the object named in the trigger node's objectType).
- {!recordId} â€” the triggering record's raw Id.
- {!ai.score}, {!ai.priority}, {!ai.finalText} â€” the most recent AI node's output (always these exact names, regardless of node label).
- {!action.records}, {!action.count} â€” the most recent action-type node's output (query_records puts its results here).
- {!<name>.value} â€” a Set Variable node's stored value, where <name> is whatever variableName you gave it.
- {!<iteratorVar>.FieldName} â€” inside a Loop's each-body, the current item (iteratorVar is whatever you named it on the loop node).

GRAPH STRUCTURE RULES:
- Exactly one trigger node, always node index 0, always type="trigger" subType="record".
- Connections reference nodes by ARRAY INDEX (fromIndex/toIndex into the nodes array you return), with fromPort/toPort â€” toPort is always "in" except loop bodies use the node's own port name as documented above.
- Every non-terminal node needs at least one outgoing connection, or the flow silently dead-ends there.
- if_else should usually wire BOTH yes and no somewhere (even if "no" just goes to a no-op end) â€” an unwired branch is a silent no-op, not an error, but it is usually a mistake.
- Catalog nodes (salesforce_crm_tools/email_tools/storage_tools/channel_tools) connect FROM the ai node they belong to, on that ai node's "out" port â€” they are the ai node's toolset, not a step in the main flow.
- Be PROACTIVE: if the requirement clearly implies structure it didn't spell out (e.g. "answer questions from our FAQ" implies useKnowledgeBase=true and possibly a KB catalog; "notify the team" without naming a channel implies a call_tool or channel_tools node using the best-matching connected provider), include it â€” but set a one-sentence "rationale" on that specific node explaining why you added it. Do not invent unrelated features.
- Never fabricate a provider that isn't Salesforce/email/storage/channel in nature â€” if the requirement needs something Archon genuinely can't do, omit it and explain the gap in the setup checklist instead of inventing a fake node type.

Decide whether to call ask_clarifying_questions or create_agent. Only ask questions when truly blocked (e.g. an approval step with no named or inferable approver, a notification with no possible channel/provider match at all) â€” prefer a reasonable default plus a checklist note over asking.`;
}
