...
export const CHAT_NODE_SPEC: NodeSpecEntry[] = [
  ...
  { type: 'catalog', subType: 'channel_tools', label: 'Channel Tools (catalog)', when: 'Attach to the ai or subagent node it belongs to so it can post to chat channels itself mid-conversation.', ports: [],
    fields: [ { key: 'description', type: 'text', description: 'Shown to the AI.' }, { key: 'connectorId', type: 'text', description: 'Leave empty string.' }, { key: 'allowedTools', type: 'string[]', description: 'Subset of: list_channels, list_users, read_channel_history, post_message, update_message, add_reaction, upload_file.' } ] },
  {
    type: 'guardrail', subType: 'guardrail', label: 'Guardrails (the agent\'s rules — ONE node per agent)',
    when: 'The agent\'s restriction rules — lines of control ("must never say X", "never below Y") ENFORCED BY THE SERVER at runtime, never prompt text. AT MOST ONE guardrail node per agent: all rules go in its config.rules array. Connected FROM the top-level ai node on its "tool" port (toPort "in"). If the agent already has a guardrail node, ADD rules to it with update_node_config (send the FULL new rules array) instead of adding another node. Also ensure the root ai node config has customerFacing=true.',
    ports: [],
    fields: [
      { key: 'rules', type: 'array', description: 'The rules list. Each entry: {"kind":"bannedWords","bannedWords":["cost price"]} OR {"kind":"numberLimit","maxDiscountField":"<verified % field on Product2>","firstOfferPct":12,"defaultMaxPct":15}. Only these two kinds exist. VERIFY any field name with describe_object first.' },
    ],
  },
  {
    type: 'automation', subType: 'automation', label: 'Automation (system does it, deterministically)',
    when: 'System work that happens automatically alongside the conversation — NOT a restriction: "when the customer mentions X, save it to a field" or "when the agent books a follow-up Task/Event, move the record\'s stage". One automation node per job, any number per agent, connected FROM the top-level ai node on its "tool" port (toPort "in"). Enforced by the server; the agent cannot skip it.',
    ports: [],
    fields: [
      { key: 'mechanism', type: 'picklist(dataCapture,followUpAction)', description: 'Which automation this node performs. Include ONLY the keys for the chosen mechanism. EXACT shapes: dataCapture = {"mechanism":"dataCapture","listenFor":"<plain-language description>","extract":["Vendor","Price","Includes"],"targetField":"<verified field on the anchored record>","keywords":["vendor","price"]}; followUpAction = {"mechanism":"followUpAction","stageField":"<verified picklist field>","fromStage":"<real picklist value>","toStage":"<real picklist value>"}.' },
      { key: 'listenFor', type: 'textarea', description: 'dataCapture only: plain-language description of what to detect in CUSTOMER messages — write it fully (what counts, what details matter).', example: 'The customer mentions a competing vendor or provider, their price, or what their offer includes.' },
      { key: 'extract', type: 'string[]', description: 'dataCapture only: names of the values to extract.', example: '["Vendor", "Price", "Includes"]' },
