...
export const NODE_SPEC: NodeSpecEntry[] = [
  ...
  },
  {
    ...
    ports: ['out'],
    fields: [
      { key: 'model', type: 'picklist(claude-opus-4-7,claude-sonnet-4-6,claude-haiku-4-5)', description: 'Pick by TIER, latency-first: FAST (mini/flash/haiku) for thin routing roots, extraction, or high-volume simple replies — lowest latency & cost; BALANCED (gpt-4o / sonnet) is the DEFAULT for customer-facing conversation; BEST (opus / o-series / *-pro) only when deep multi-step reasoning quality is worth extra latency. Default claude-sonnet-4-6.' },
  ...
  { type: 'ai', subType: 'gpt4', label: 'GPT-4 (AI step)', when: 'Same as claude — use only if the user specifically asked for OpenAI/GPT.', ports: ['out'],
    fields: [
      { key: 'model', type: 'picklist(gpt-4o,gpt-4o-mini,gpt-4-turbo,gpt-4.1,gpt-4.1-mini)', description: 'Pick by TIER, latency-first: FAST (mini/flash/haiku) for thin routing roots, extraction, or high-volume simple replies — lowest latency & cost; BALANCED (gpt-4o / sonnet) is the DEFAULT for customer-facing conversation; BEST (opus / o-series / *-pro) only when deep multi-step reasoning quality is worth extra latency. Default gpt-4o.' },
  ...
  { type: 'ai', subType: 'gemini', label: 'Gemini (AI step)', when: 'Same as claude — use only if the user specifically asked for Gemini/Google AI.', ports: ['out'],
    fields: [
      { key: 'model', type: 'picklist(gemini-2.5-pro,gemini-2.5-flash,gemini-2.0-flash,gemini-2.0-flash-lite)', description: 'Pick by TIER, latency-first: FAST (mini/flash/haiku) for thin routing roots, extraction, or high-volume simple replies — lowest latency & cost; BALANCED (gpt-4o / sonnet) is the DEFAULT for customer-facing conversation; BEST (opus / o-series / *-pro) only when deep multi-step reasoning quality is worth extra latency. Default gemini-2.5-flash.' },
      { key: 'instruction', type: 'textarea', description: 'Same as claude.instruction.' },
      { key: 'useKnowledgeBase', type: 'toggle', description: 'Same as claude.useKnowledgeBase.' },
    ] },
  {
...
export const CHAT_NODE_SPEC: NodeSpecEntry[] = [
  ...
  },
  {
    ...
    fields: [
      ...
      { key: 'actionType', type: 'picklist(Prebuilt,MCP,Apex,Flow)', description: 'PREFER "Prebuilt" whenever the action is a known CRUD step on a KNOWN object (create/update/get/search with specific fields) — it becomes a typed, scoped tool. Use MCP for exploratory/unknown-object access; Apex/Flow only when the requirement names an internal action.' },
      { key: 'operation', type: 'picklist(create,update,get,search)', description: 'Prebuilt only.' },
      { key: 'object', type: 'text', description: 'Prebuilt only: the object API name — VERIFY with describe_object first.' },
      { key: 'selectedFields', type: 'array', description: 'Prebuilt only: the fields the AI may use, each as {"name","label","type","required"} taken from describe_object — never invented.' },
